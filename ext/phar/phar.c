...
/* $Id: phar.c,v 1.104 2007/01/09 18:54:36 helly Exp $ */
...
static int phar_flush(phar_entry_data *data TSRMLS_DC) /* {{{ */
{
	...
	phar_entry_info *entry;
	int alias_len, fname_len, halt_offset, restore_alias_len, global_flags = 0, close_fp;
	...
	if (data->phar->halt_offset && data->fp) {
	...
	for (zend_hash_internal_pointer_reset(&data->phar->manifest);
		...
		if (entry->flags & PHAR_ENT_COMPRESSION_MASK) {
			...
			if (bufsize < entry->uncompressed_filesize) {
				...
				bufsize = entry->uncompressed_filesize;
			}
...
PHP_MINFO_FUNCTION(phar) /* {{{ */
{
	...
	php_info_print_table_row(2, "CVS revision", "$Revision: 1.104 $");
